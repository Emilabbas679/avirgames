...
@section('title', 'Edit Discussion category')

@section('content')

    <div class="card shadow mb-4">
        <div class="card-header py-3 d-sm-flex align-items-center justify-content-between mb-4">
            <h6 class="m-0 font-weight-bold text-primary">Edit {{ $category->name }}</h6>
        </div>
        <div class="card-body">
            <div class="col-md-6 offset-md-3">
                <form class="user" method="POST" action="{{ route('discussion-category.update', $category->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                                ...
                                <div class="form-group form-row">
                                    ...
                                    <div class="col-md-9">
                                        <input id="question" type="text" class="form-control @error('name') is-invalid @enderror"
                                               name="name[{{ $locale->code }}]" value="{{ $category->getTranslation('name', $locale->code) }}"
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" name="icon" value="{{$category->icon}}" class="form-control @error('icon') is-invalid @enderror">
                            @error('icon')
                            <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
